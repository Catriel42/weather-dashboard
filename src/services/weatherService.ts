...
export const searchCities = async (query: string) => {
  try {
    const response = await fetch(`${BASE_URL_GEO}/direct?q=${query}&limit=5&appid=${API_KEY}`);
...
export const getCurrentWeather = async (lat: number, lon: number) => {
  try {
    const response = await fetch(`${BASE_URL_WEATHER}/weather?lat=${lat}&lon=${lon}&units=metric&appid=${API_KEY}`);
